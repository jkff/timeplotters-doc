...
% TODO Make image top-aligned with text.
\newcommand{\inlinethumbnail}[1]{\includegraphics[width=0.12\textwidth]{#1}}
...
% TODO Labels for indiviual kinds
...
\begin{itemize}
 \item Make a dot plot of \verb|Voltage| for January 1st, 2007. Do not limit \emph{the trace}, use \verb|-fromTime| and \verb|-toTime|. It will look like this: \inlinethumbnail{pics/tplot/power-voltage.png}
 \item Make a line plot of \verb|Global_active_power| vs \verb|Global_reactive_power| on one plot and a line plot of \verb|Voltage| on another (i.e., the result should be a stack of two plots), for January 1st, 2007. Use an opacity level of 0.5. It will look like this: \inlinethumbnail{pics/tplot/power-active-reactive-voltage.png}
 \item Make a quantile plot, with quantiles of 0.1, 0.5 and 0.9 over bins of 1 day, for \verb|Global_active_power| and \verb|Global_reactive_power|, for the entire dataset. It will look like this: \inlinethumbnail{pics/tplot/power-active-reactive-quantile.png}
\end{itemize}
...
\begin{itemize}
 \item Plot the number of samples arriving per second to \verb|Grab1|, \verb|Grab2|, \verb|SR.YUV| and \verb|AviWr|. The result will look like this: \inlinethumbnail{pics/tplot/thedeemon-acount.png}
 \item Make an event plot, using impulse events for sample arrivals to these stages. Limit to the first 20 seconds. The result will look like this: \inlinethumbnail{pics/tplot/thedeemon-event.png}
 \item Make 4 dot plots for durations of processing at these stages. The result will look like this: \inlinethumbnail{pics/tplot/thedeemon-dots.png}
 \item Plot the same, but on a single plot (hint: use \verb|within|). The result will look like this: \inlinethumbnail{pics/tplot/thedeemon-dots-all.png}
\end{itemize}
...
\begin{itemize}
 \item Plot the data arrival rate (according to \verb|data arrived| entries) per 5-second bins (use \verb|sum|). The result should look like this: \inlinethumbnail{pics/tplot/graphov-arrival.png}
 \item Plot the transaction commit rate (according to \verb|Successful COMMIT| entries) per 5-second bins. The result should look like this: \inlinethumbnail{pics/tplot/graphov-txcommit.png}
 \item Plot the same transaction commit rate, but so that we can see how it adds up from contributions by different threads (\verb|mth:1|, \verb|pth:2| etc.). Limit to the interval from 12:00 to 13:00. The result should look like this: \inlinethumbnail{pics/tplot/graphov-txcommit-bythread.png}
 \item Same as above, \emph{and use the same trace file}, but make it 1 plot per thread type (\verb|mth|, \verb|pth|, \verb|dbv| etc., but still with a breakdown \emph{by thread} within each type's plot). The result should look like this: \inlinethumbnail{pics/tplot/graphov-txcommit-bythreadtype.png}
\end{itemize}
