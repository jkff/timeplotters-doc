...
\begin{wrapfigure}[4]{c}{70pt}
\vspace{-25pt}
\center
\includegraphics[height=50pt]{pics/tplot/dots.png}
\end{wrapfigure}
...
\vspace{-25pt}
\center
\includegraphics[height=50pt]{pics/tplot/acount.png}
\end{wrapfigure}
...
\vspace{-25pt}
\center
\includegraphics[height=40pt]{pics/tplot/dist-quantile.png}
\end{wrapfigure}
...
\begin{wrapfigure}[5]{c}{70pt}
\vspace{-25pt}
\center
\includegraphics[height=50pt]{pics/tplot/binf-binh.png}
\end{wrapfigure}
...
\vspace{-25pt}
\center
\includegraphics[height=50pt]{pics/tplot/tplot-three-jobs.png}
\end{wrapfigure}
...
In this section we list some example datasources and exercises to try on them. \textbf{WARNING:} The section \nameref{sec:tplot-gallery} has solutions to many of the exercises. Don't look at graphs marked ``exercise'' if you don't want a spoiler.
...
Make a quantile plot, with quantiles of 0.1, 0.5 and 0.9 over bins of 1 day, for \hlverb{Global\_active\_power} and \hlverb{Voltage}, for the entire dataset. & \thumbnail{pics/tplot/power-voltage-quantile.png} \\
\hline
\end{tabular}
...
Look for plots that seem like what you want. Consult sections \nameref{sec:tplot-input-format}, \nameref{sec:tplot-plot-kinds} and \nameref{sec:tplot-track-mapping}.

% Simplest numeric plots and basic track mapping
\begin{figure}[H]
\centering
\subfigure[A dot plot of request execution times. 1 input track, 1 output track. Events look like \hlverb{=arc 1542}, plot type \hlverb{-dk dots}. Data provided by Julia Astakhova]{\includegraphics[width=0.4\textwidth]{pics/tplot/dots.png}}
\quad
\subfigure[A dot plot of two parts of request execution times. 2 input tracks, 1 output track. Events look like \hlverb{=t.createUser 25} or \hlverb{=t.createProfile 7}, plot type \hlverb{-dk 'within[.] dots'}. Data provided by Julia Astakhova.]{\includegraphics[width=0.4\textwidth]{pics/tplot/dots-create-user-and-profile.png}}
\\
\subfigure[Same as above but with transparency. Plot type \hlverb{-dk 'within[.] dots 0.2'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/dots-create-user-and-profile-alpha.png}}
\quad
\subfigure[A line plot. 2 input tracks, 2 output tracks. Events look like \hlverb{>client.THREADID}, \hlverb{<client.THREADID}, \hlverb{>gateway.THREADID} and \hlverb{<gateway.THREADID}, plot type \hlverb{-dk 'within[.] duration drop lines'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/lines.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\subfigure[Distribution of page download times by a web crawler~--- how many take $<100$ms, $100..500$ etc. this is actually 2 different picture stitched together. each had 1 input track with events like \hlverb{=time 254.1}. drawn with \hlverb{-dk 'binh 900'} and \hlverb{-dk 'binf 900'} correspondingly (15-minute slices).]{\includegraphics[width=0.4\textwidth]{pics/tplot/binf-binh.png}}
\quad
\subfigure[Distribution of request execution times and rate of request completions per minute in an akka program. events look like \hlverb{>akka.THREADID}, \hlverb{<akka.THREADID} and \hlverb{!finish}, plot type \hlverb{+k akka '+b within[.] duration drop binf 60 0.01,0.1,1,2,5' +k akka '+q within[.] duration drop quantile 60 0.25,0.5,0.75' -k finish 'acount 60'}. data provided by arnaud bailly.]{\includegraphics[width=0.4\textwidth]{pics/tplot/dist-quantile.png}}
\\
\subfigure[Change of some super-secret value at Yandex over time: per-day min, max and median are shown. Events look like \hlverb{=foo 9350}, plot type \hlverb{-dk 'quantile 86400 0.5'}.]{\includegraphics[width=0.4\textwidth]{pics/tplot/median.png}}
\quad
\subfigure[\textbf{EXERCISE.} Per-day quantiles of power and voltage consumed by 1 household for a year. Events look like \hlverb{=power 5.6} and \hlverb{=voltage 245.2}, plot type \hlverb{-dk 'quantile 86400 0.1,0.5,0.9'}.]{\includegraphics[width=0.4\textwidth]{pics/tplot/power-voltage-quantile.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\subfigure[Absolute and relative number of task completions of two on a cluster per second. Events look like \hlverb{=absolute `244} and \hlverb{=relative `244} (this was before \timeplot{} had the \hlverb{+k} construction, so events had to be duplicated), plot type \hlverb{-k absolute 'hist 1' -k relative 'freq 1'}.]{\includegraphics[width=0.8\textwidth]{pics/tplot/freq-hist.png}}
\\
\subfigure[Number of different exception types encountered by a web crawler. Events look like \hlverb{=Code `java.net.SocketException}, plot type \hlverb{-dk 'hist 60'}.]{\includegraphics[width=0.8\textwidth]{pics/tplot/download-exceptions-hist.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\subfigure[Activity of 8 ``computation sites'' to be processed. Events look like \hlverb{>site-5}, \hlverb{<site-5}, plot type \hlverb{-dk event}.]{\includegraphics[width=0.2\textwidth]{pics/tplot/event.png}}
\quad
\subfigure[\textbf{EXERCISE.} Frame arrival times to several stages of a video encoding pipeline. Events look like \hlverb{!Grab2}, plot type \hlverb{-dk event}.]{\includegraphics[width=0.4\textwidth]{pics/tplot/thedeemon-event.png}}
\\
\subfigure[Number of concurrently running tasks on a cluster. Events look like \hlverb{>running}, \hlverb{<running}, plot type \hlverb{-dk 'acount 5'}.]{\includegraphics[width=0.4\textwidth]{pics/tplot/tplot-job-parallelization.png}}
\quad
\subfigure[Number of tasks started and finished per second on a cluster. Events look like \hlverb{!in} and \hlverb{!out}, plot type \hlverb{-dk 'acount 1'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/acount.png}}
\\
\subfigure[Number of tasks started, finished and concurrently running per second on a cluster. Events look like \hlverb{!begin/5s}, \hlverb{!end/5s}, \hlverb{>running} and \hlverb{<running}, plot type \hlverb{-dk 'acount 5'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/acount-begin-end-running.png}}
\quad
\subfigure[Number of tasks received, completed and submitted (with breakdown by ``task type''~--- ``simulation'' or ``slabbing'') on a cluster. Events look like \hlverb{!got}, \hlverb{!completed}, \hlverb{!submit.Simulation} and \hlverb{!submit.Slabbing}, plot type \hlverb{-k submit 'within[.] acount 1' -dk 'acount 1'}.]{\includegraphics[width=0.4\textwidth]{pics/tplot/rates.png}}
\end{figure}
...
\begin{figure}[H!]
\centering
\subfigure[Number of tasks ``in queue'' and ``in progress'' on a cluster, with breakdown by job id. Events look like \hlverb{>fly.JOBID}, \hlverb{<fly.JOBID}, \hlverb{>run.JOBID} and \hlverb{<run.JOBID}, plot type \hlverb{-dk 'within[.] acount 5'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/acount-fly-run-manyjobs.png}}
\quad
\subfigure[\textbf{EXERCISE.} Number of DB transaction commits per second performed from several threads. Events look like \hlverb{!tx.mth:3}, plot type \hlverb{-dk 'within[.] acount 1'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/graphov-txcommit-bythread.png}}
\\
\subfigure[\textbf{EXERCISE.} Number of DB transaction commits per second performed from several threads, classified by thread type. Events look like \hlverb{!tx.mth:3}, plot type \hlverb{-dk 'within[:] acount 1'}]{\includegraphics[width=0.4\textwidth]{pics/tplot/graphov-txcommit-bythreadtype.png}}
\end{figure}
...
Please carefully examine how this corresponds to the trace above. This has no particular meaning~--- it's merely an illustration of the different types of events in the trace. The tracks are ordered by time of the first event, so \hlverb{thread-2} is the top track on this picture.
...
\item[Color] Colors in \splot{} can be specified in several ways. In the simplest form, it can be an SVG color name or hex code (e.g. \hlverb{red} or \hlverb{lightblue} or \hlverb{\#ff0033}), it can be an arbitrary string (then a random color will be generated so that different strings correspond to different colors~--- this is useful to color-code an unknown number of different types of activities, e.g. if you have worker processes servicing several clients and you wish to get a picture of who services whom and color-code the clients: \hlverb{client-5}), and it can be an arbitrary string within a color scheme (e.g. \hlverb{/success/client-5} or \hlverb{/failure/client-5}, and you might define the \hlverb{success} colorscheme to consist of several greenish colors and \hlverb{failure} of reddish).
...
\begin{itemize}
 \item \hlverb{TIME >TRK COLOR}~--- Start activity of color COLOR on track TRK at time TIME (if there already is an activity, finish it and start a new one instead). \\
\textbf{Example:} \texttt{2010-10-21 16:45:09,431 >r2b3.t5 blue}
 \item \hlverb{TIME <TRK}~--- Finish the current activity of track TRK at time TIME. \\
\textbf{Example:} \texttt{2010-10-21 16:45:10,322 <r23.t5} 
 \item \hlverb{TIME <TRK COLOR}~--- Finish the current activity of track TRK at time TIME, overriding the color it was given at the start by COLOR. This is useful, e.g. do indicate that an activity failed by drawing it with red color (obviously when the activity begins, we don't know if it will fail or not). \\
\textbf{Example}: \texttt{2010-10-21 16:45:10,322 <r2b3.t5 red} 
 \item \hlverb{TIME !TRK COLOR TXT}~--- Draw text TXT with color COL on track TRK, left-justified at time TIME. \\
...
The trace looks like this: here, we use client ids as color codes (\hlverb{8610}, \hlverb{877B} etc.)
\begin{verbatim}
...
...
...
\end{verbatim}
So, we use track names of the form \hlverb{MACHINE.WORKERID} and instead of using color at \hlverb{>} events, we use the client ID, asking \splot{} to color-code clients for us. Here red color denotes tasks that completed unsuccessfully and other colors (e.g. green and blue; red is excluded from the default color scheme precisely for situations like this) encode different clients\footnote{Yes, as this rather crazy picture might hint, the program \emph{did} have lots of problems~--- exposing them, that's what \splot{} is for.}.
